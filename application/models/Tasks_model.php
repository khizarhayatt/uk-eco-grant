...
class Tasks_model extends App_Model
{
    ...
    public function add($data, $clientRequest = false)
    {
        ...
        if (is_array($data['notify_method'])) {
            // Assuming you want to store the methods as a comma-separated string
            $data['notify_method'] = implode(',', $data['notify_method']);
        } else {
            // Handle single selection case or default value
            if ($data['notify_method']=== 1) {
                $data['notify_method'] = 1;
            } elseif ($data['notify_method']=== 2) {
                $data['notify_method'] = 2;
            } elseif ($data['notify_method']=== 3) {
                $data['notify_method'] = 3;
            } else {
                $data['notify_method'] = 0; // Default value if none of the conditions match
            }
        }
