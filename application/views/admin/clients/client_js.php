...
$('#status').change(function() {
    ...
    $('input[name="dwt_status"]').change(function() {
        var selectedStatus = $(this).val();
        var formAction = $('#client_status_form').attr('action');
        var csrfToken = $('input[name="csrf_token_name"]').val();
        $(window).off("beforeunload");
    ...
    function updateSurveyStatus(checkbox) {
        var statusType = checkbox.value;
        var statusValue = checkbox.checked ? 1 : 0;

        $.ajax({
            url: '<?php echo admin_url('clients/survey_update_ajax'); ?>', // URL to your PHP handler
            type: 'POST',
            data: {
                customer_id: <?php echo $client->userid; ?>, // Pass the user ID 
    ...
    function updateWorkflowStatus(checkbox) {
        var statusType = checkbox.value;
        var statusValue = checkbox.checked ? 1 : 0;

        $.ajax({
            url: '<?php echo admin_url('clients/workflow_update_ajax'); ?>', // URL to your PHP handler
            type: 'POST',
            data: {
                customer_id: <?php echo $client->userid; ?>, // Pass the user ID 
                status_type: statusType,
    ...
    function dwpStatus(checkbox) {
        var statusType = checkbox.value;
        var statusValue = checkbox.checked ? 1 : 0;

        $.ajax({
            url: '<?php echo admin_url('clients/dwp_update_ajax'); ?>', // URL to your PHP handler
            type: 'POST',
            data: {
                customer_id: <?php echo $client->userid; ?>, // Pass the user ID 
                status_type: statusType,
