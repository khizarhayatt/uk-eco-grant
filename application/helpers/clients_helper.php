...
if (!function_exists('get_workflow_count_by_customer_id')) {
    function get_workflow_count_by_customer_id($customer_id) 
    {
        // Get an instance of CodeIgniter
        $CI =& get_instance();
        ...
        // Query the database to get the count of all workflow statuses for the given customer ID
        $CI->db->where('customer_id', $customer_id);
        $CI->db->from('tblclient_worflow_statuses');
...
if (!function_exists('get_last_wf_status_value_by_customer_id')) {
    function get_last_wf_status_value_by_customer_id($customer_id) 
    {
        // Get an instance of CodeIgniter
        $CI =& get_instance();
        ...
        // Query the database to get the last updated status value
        $CI->db->select('status_type');  
        $CI->db->where('customer_id', $customer_id);
        $CI->db->order_by('date_updated', 'DESC');
        $CI->db->limit(1); // Get only the last updated record
        $query = $CI->db->get('tblclient_worflow_statuses');

        // Return the status_value
        $row = $query->row();
        if ($row) {
            // Convert the status value to a sentence
            $status_value = ucfirst(str_replace('_', ' ', $row->status_type)) . '.';
            return $status_value;
        } else {
...
if (!function_exists('get_last_survey_status_value_by_customer_id')) {
    function get_last_survey_status_value_by_customer_id($customer_id) 
    {
        // Get an instance of CodeIgniter
        $CI =& get_instance();
