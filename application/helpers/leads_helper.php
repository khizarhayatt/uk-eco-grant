...
function render_customer_status_radio($selected = '', $lang_key = '', $name = 'dwt_status',  $exclude_default = false)
{
    // Static statuses
    $statuses = [
    ...
    // Loop through each status and create a radio button
    foreach ($statuses as $status) {
        $checked = ($selected == $status['id']) ? 'checked' : '';
        $radio_buttons .= '<label>';
        $radio_buttons .= '<input type="radio" name="' . htmlspecialchars($name) . '" value="' . htmlspecialchars($status['id']) . '" ' . $checked . '>';
        $radio_buttons .= htmlspecialchars($status['name']);
        $radio_buttons .= '</label><br>';
    }
...
function render_sceme_select($selected = '', $include_blank = true) {
    $options = [
        ['id' => 'GBIS', 'name' => 'GBIS'],
        ['id' => 'ECO', 'name' => 'ECO'],
        ['id' => 'LA-Flex', 'name' => 'LA-Flex'],
        ['id' => 'Private', 'name' => 'Private'],
    ];

    $select_html = render_select(
        $name = 'scheme',
        $options = $options,
        $option_attrs = ['id', 'name'],
        $label = 'Scheme',
        $selected = $selected, // Default selected option
        $select_attrs = [],
        $form_group_attr = [],
        $form_group_class = '',
        $select_class = 'lead_sceme',
