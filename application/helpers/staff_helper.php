...
function get_staff_full_names($userids = array())
{
    ...
    foreach ($userids as $userid) {
        ...
        $tmpStaffUserId = get_staff_user_id();
        if ($userid == $tmpStaffUserId) {
            if (isset($GLOBALS['current_user'])) {
                $result[$userid] = $GLOBALS['current_user']->firstname . ' ' . $GLOBALS['current_user']->lastname;
                continue;
            }
            $userid = $tmpStaffUserId;
        }
        ...
        if ($staff) {
            $result[$userid] = $staff->firstname . ' ' . $staff->lastname;
        } else {
            $result[$userid] = '';
        }
    }
...
function get_staff_phonenumbers($userids = array())
{
    // Ensure $userids is an array
    if (!is_array($userids)) {
        $userids = array($userids);
    ...
    foreach ($userids as $userid) {
        ...
        if ($userid == $tmpStaffUserId) {
            if (isset($GLOBALS['current_user'])) {
                $result[$userid] = $GLOBALS['current_user']->phonenumber;
                continue;
            }
            $userid = $tmpStaffUserId;
        }
        ...
        if ($staff) {
            $result[$userid] = $staff->phonenumber;
        } else {
            $result[$userid] = '';
        }
    }
