...
class Clients extends AdminController
{
    ...
    public function survey_update($client_id = null)
    {
        if ($this->input->post()) {
            ...
            // Define the statuses
            $statuses = [
                'awaiting_doc_verification' => $this->input->post('awaiting_doc_verification') ? 1 : 0,
                'awaiting_epr' => $this->input->post('awaiting_epr') ? 1 : 0,
                'awaiting_dwp' => $this->input->post('awaiting_dwp') ? 1 : 0,
                'awaiting_installation' => $this->input->post('awaiting_installation') ? 1 : 0,
                'awaiting_submission' => $this->input->post('awaiting_submission') ? 1 : 0,
            ];

           
            // Insert or update each status
            $success = $this->clients_model->update_survey_status($client_id , $statuses, $user_id);
            
            if ($success) {
                set_alert('success', _l('updated_successfully', _l('client')));
            } else {
                
                set_alert('danger', "Not Updated " );
                error_log("Failed to update survey status for client ID: $client_id by user ID: $user_id");
                
            }
            // Redirect back to the client profile or survey list
            
            redirect(admin_url('clients/client/' . $client_id));
        } else {
            // Handle GET request or form rendering
            set_alert('erroe', _l('updated_successfully', "Data not inserted"));
            redirect(admin_url('clients/client/' . $client_id));
        }
    } 
    
    public function survey_update_ajax()
    {
        if ($this->input->is_ajax_request()) {
            // Retrieve the data sent via AJAX
            $status_type = $this->input->post('status_type');
            $customer_id = $this->input->post('customer_id');
            $status_value = $this->input->post('status_value') ? 1 : 0;
            $user_id = get_staff_user_id(); // Get the current logged-in user ID
            ...
            $statuses = [
                ...
                'date_updated' => $current_timestamp, // Include the timestamp
                'user_id' => $user_id
            ];
    
            // Call the model function to update/insert the status
            $success = $this->clients_model->update_survey_status_ajax($customer_id, $status_type, $statuses);
    
            // Respond to the AJAX request
            if ($success) {
                echo json_encode(['success' => true, 'message' => 'Status updated successfully']);
            } else {
                error_log("Failed to update survey status for client ID: $customer_id by user ID: $user_id");
                echo json_encode(['success' => false, 'message' => 'Failed to update status']);
            }
        } else {
            // If not an AJAX request, show an error
            error_log("Invalid request type for updating survey status for client ID: $customer_id");
            show_error('No direct access allowed');
        }
    }


    public function workflow_update_ajax()
    {
        if ($this->input->is_ajax_request()) {
            // Retrieve the data sent via AJAX
            $status_type = $this->input->post('status_type');
            ...
            // Call the model function to update/insert the status
            $success = $this->clients_model->update_workflow_status_ajax($customer_id, $status_type, $statuses);
    
            // Respond to the AJAX request
            if ($success) {
                echo json_encode(['success' => true, 'message' => 'Status updated successfully']);
            } else {
                error_log("Failed to update worflow status for client ID: $customer_id by user ID: $user_id");
                echo json_encode(['success' => false, 'message' => 'Failed to update status']);
            }
        } else {
            // If not an AJAX request, show an error
            error_log("Invalid request type for updating worflow status for client ID: $customer_id");
            show_error('No direct access allowed');
        }
    }
    public function dwp_update_ajax()
    {
        if ($this->input->is_ajax_request()) {
            // Retrieve the data sent via AJAX
            $status_type = $this->input->post('status_type');
            ...
            // Call the model function to update/insert the status
            $success = $this->clients_model->update_dwt_status_ajax($customer_id, $status_type, $statuses);
