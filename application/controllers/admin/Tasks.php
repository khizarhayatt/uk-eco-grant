...
class Tasks extends AdminController
{
    ...
    public function task($id = '')
    {
        ...
        if ($this->input->post()) {
            ...
            $assignees = isset($data['assignees']) ? $data['assignees'] : null;
            $followers = isset($data['followers']) ? $data['followers'] : null;
            $description = isset($data['description']) ? $data['description'] : null;
            $plain_description = strip_tags($description);
            $dates = $data['startdate'] . '-' . $data['duedate'];
            if ($priority == 1) {
                $priorityText = 'low';
            } elseif ($priority == 2) {
                $priorityText = 'medium';
                ...
                $priorityText = 'urgent'; // Default case if the priority value doesn't match any known values
            } else {
                $priorityText = '-';
            }
            //    log_message('error' ,  print_r($method, true) );

            $assigneesStr = implode(', ', get_staff_full_names($assignees));
            $followersStr = implode(', ', get_staff_full_names($followers));
            $assigneesNum = get_staff_phonenumbers($assignees);
            $followersNum = get_staff_phonenumbers($followers);
            // Merge both arrays
            $mergedArray = array_merge($assigneesNum, $followersNum);
            // Remove duplicates
            $WAnums = array_unique($mergedArray);
            $lead = get_lead_by_id($rel_id);
            ...
            // Add the lead information only if the lead is available
            ...
            $msgwrapwa =
                " Name: $name " . " | Dates: $dates " . " | Priority: $priorityText ";
            // Add the lead information only if the lead is available
            if (isset($lead) && $lead !== null) {
                $msgwrapwa .= " | Lead: $lead->id - $lead->name ";
            }
            $msgwrapwa .=  " | Assignees: $assigneesStr " . " | Followers: $followersStr " . " | Description: $plain_description";
            ...
            if ($id == '') {
                ...
                //  log_message('error', print_r(  $lead->phonenumber, true));


                if (isset($data['is_appointment'])) {

                    log_message('error', 'if : app:' . $data['is_appointment']);

                    if (isset($data['notify_method']) && is_array($data['notify_method'])) {
                        if (in_array(1, $data['notify_method'])) {

                            //send_sms_lead_message( $lead->phonenumber, "test" );
                            send_sms_lead_message($lead->phonenumber, $msgwrapsms);
                            log_message('error', "sms to lead- done");
                        }
                        if (in_array(2, $data['notify_method'])) {

                            send_whatsapp_lead_message($lead->phonenumber,  $msgwrapwa);
                            //   send_whatsapp_lead_message( $lead->phonenumber, $msgwrapwa );
                            log_message('error', "whatsapp to lead- done");
                        }
                        if (in_array(3, $data['notify_method'])) {
                            log_message('error', "email to lead");
                        }
                    }
                }
                ...
                if (isset($WAnums)) {
                    ...
                    log_message('error', print_r($WAnums, true));
                    send_sms_task_messages($WAnums, $msgwrapsms   ); 
                    log_message('error', "SMS to TEAM- done");
                   // send_sms_task_messages($WAnums, $msgwrapwa);
                    send_whatsapp_task_messages($WAnums, $msgwrapwa);
                    log_message('error', "whatsapp to TEAM- done");
                }
        ...
        // Iterate through members and collect emails and phone numbers
        foreach ($data['members'] as $member) {
            if (isset($member['email'])) {
                $emails[] = $member['email'];
            }
            if (isset($member['phonenumber'])) {
                $phone_numbers[] = $member['phonenumber'];
            }
        }
        ...
        if (isset($phone_numbers)) {
            ...
            //send_whatsapp_task_message($phone_numbers, $msgwrap   );
