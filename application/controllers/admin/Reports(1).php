...
class Reports extends AdminController
{
    ...
    /* Repoert leads by status */
    public function leadsReportByStatus($id)
    {
        $data= json_encode($this->reports_model->leads_sources_report_by_status($id)); 
        echo $data;
    }
    ...
    /* Workflows Reports */
    public function wokflows_report()
    { 
        $data['statuses'] = $this->leads_model->get_status();
    ...
    public function send_sms() {
        ...
        private function send_custom_sms_message($to, $message, $des) {
            ...
            $contentSid = 'HX501c9988a6026f1d257e5b57620e37ec';
            ...
            try {
                ...
                if (preg_match('/^\+44\d{10,14}$/', $cleanedNumber)) {
                    // Ensure $des is a string
                    $desString = is_string($des) ? $des : strval($des);
                    
                    // Prepare contentVariables
                    $contentVariables = json_encode(['1' => $desString], JSON_FORCE_OBJECT);
        
                    // For debugging
                    error_log("Content Variables: " . $contentVariables);
                    ...
                    // Only add contentSid and contentVariables if $des is not empty
                    if (!empty($desString)) {
                        $messageParams['contentSid'] = $contentSid;
                        $messageParams['contentVariables'] = $contentVariables;
                    }
        
                    $client->messages->create($cleanedNumber, $messageParams);
                } else {
