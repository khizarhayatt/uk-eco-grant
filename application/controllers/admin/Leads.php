...
class Leads extends AdminController
{
    ...
    public function lead($id = '')
    {
        ...
        if ($this->input->post()) {
            ...
            if ($id == '') {
                ...
                $lead_url_pattern = '/admin/clients/client/';
                $current_url = $_SERVER['REQUEST_URI'];
                if (strpos($current_url, $lead_url_pattern) !== false) {
                    echo json_encode([
                        'success'  => $id ? true : false,
                        'id'       => $id,
                        'message'  => $message,
    ...
    public function lead_survey_update($id = '')
    {
        if (!is_staff_member() || ($id == '' || !$this->leads_model->staff_can_access_lead($id))) {
            ajax_access_denied();
        }
    ...
    private function _get_lead_data($id = '')
    {
        ...
        if (is_numeric($id)) {
            ...
            $data['comments']         = $this->misc_model->get_comments($id, 'lead');
            ...
            // $data['total_comments']       = $leadProfileBadges->getCount('comments');
    ...
    public function convert_to_customer()
    {
        ...
        if ($this->input->post()) {
            ...
            if (isset($data['transfer_comments'])) {
                $comments = $this->misc_model->get_comments($data['leadid'], 'lead');
                unset($data['transfer_comments']);
            }
                ...
                if (isset($comments)) {
                    foreach ($comments as $comment) {
                        $this->db->insert(db_prefix() . 'notes', [
                            ...
                            'addedfrom'      => $comment['addedfrom'],
                            'description'    => $comment['description'],
                            'date_contacted' => $comment['date_contacted'],
                            'is_comment' => 1, 
                        ]);
                    }
                }
    ...
    public function delete_comment($id, $lead_id)
    {
        if (!is_staff_member() || !$this->leads_model->staff_can_access_lead($lead_id)) {
            ajax_access_denied();
        }
        echo json_encode([
            'success'  => $this->misc_model->delete_comment($id),
            'leadView' => $this->_get_lead_data($lead_id),
            'id'       => $lead_id,
        ]);
    }
