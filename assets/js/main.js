...
$(function () {
  ...
  $("body").on("shown.bs.modal", "#_task_modal", function (e) {
    ...
    // On shown lead add/edit modal
    $("body").on("shown.bs.modal", "#_lead_modal", function (e) {
      if (!$(e.currentTarget).hasClass("edit")) {
        $("body").find("#_lead_modal #name").focus();
      } else {
        if ($(this).find(".tinymce-lead").val().trim() !== "") {
          init_editor(".tinymce-lead", {
            height: 200,
          });
        }
      }
...
function edit_comment(id) {
  var editor = tinymce.get('note'); // Get TinyMCE editor instance for the textarea with id 'note'
  var description = editor ? editor.getContent() : ""; // Get content from TinyMCE

  if (description !== "") {
    $.post(admin_url + "misc/edit_comment/" + id, {
