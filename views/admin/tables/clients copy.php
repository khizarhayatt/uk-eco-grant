...
return App_table::find('clients')
    ->outputUsing(function ($params) {
        ...
        $aColumns = [
            ...
            db_prefix() . 'clients.status as status',
        ];
        ...
        foreach ($rResult as $aRow) {
           ...
           // $row[] = ($aRow['contact_id'] ? '<a href="' . admin_url('clients/client/' . $aRow['userid'] . '?contactid=' . $aRow['contact_id']) . '" target="_blank">' . e(trim($aRow['fullname'])) . '</a>' : '');
            ...
            $row[] =  $aRow['address'] ?     strip_tags($aRow['address']) .' ' . e($aRow['city']).' ' . e($aRow['state']).' ' . e($aRow['zip'])    : '';
    ...
    <label class="onoffswitch-label" for="' . $aRow['userid'] . '"></label>
            ...
            if ($aRow['status'] + 1 == 1) {
                $status = 'Under EPR';
            } elseif ($aRow['status'] + 1 == 2) {
                $status = 'Under Installation';
            } elseif ($aRow['status'] + 1 == 3) {
                $status = 'Under Submission';
            } elseif ($aRow['status'] + 1 == 4) {
                $status = 'Completed';
            } else {
                $status = 'Unknown';
            }
            $row[] = $status;
//
            $surveys =   count_non_empty_custom_fields( $aRow['leadid'] );   
            switch ($surveys) {
                case '1':
                    $row[] = "Awaiting Doc Verification";
                    break;
                case '2':
                    $row[] = "Awaiting EPR";
                    break;
                case '3':
                    ...
                    break;
                case '4':
                    $row[] = "Awaiting Installation";
                    break;
                case '5':
                    $row[] = "Awaiting Submission";
                    break;
                default:
                    $row[] = "-"; // or handle default case as needed
                    break;
            }
