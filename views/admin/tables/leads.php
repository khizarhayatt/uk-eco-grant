...
$rules[] = App_table_filter::new('assigned', 'SelectRule')->label(_l('leads_dt_assigned_to'))
    ->withEmptyOperators()
...
return App_table::find('leads')
    ->outputUsing(function ($params) use ($statuses) {
        ...
        $aColumns = [
            ...
            db_prefix() . 'leads.assignees as assignees',
        ];
        ...
        foreach ($rResult as $aRow) {
            ...
            $row[] = ($aRow['address'] != '' ?  e($aRow['address']) .', ' . e($aRow['city']).', ' . e($aRow['state']).', ' . e($aRow['zip'])    : '');
           ...
           // $row[]         = e(($aRow['lead_value'] != 0 ? app_format_money($aRow['lead_value'], $base_currency->id) : ''));
            ...
            $assignees=[];
            $assigneeNames = [];
            if (isset($aRow['assignees'])) {
                $assignees = is_array($aRow['assignees']) ? $aRow['assignees'] : explode(', ', $aRow['assignees']);
                
                foreach ($assignees as $assignee) {
                   
                     $assigneeNames[] = '<a data-toggle="tooltip" data-title="' .get_staff_full_name(trim($assignee)) . '" href="' . admin_url('profile/' . $assignee) . '">' . staff_profile_image($assignee, [
                               'staff-profile-image-small',
                               ]) . '</a>';
                } 
                $assignedOutput = $assigneeNames;
