...
class Clients_model extends App_Model
{
    ...
    */
    ...
    public function update_survey_status($customer_id, $statuses, $user_id)
    {
        foreach ($statuses as $status_type => $status_value) {
            // Prepare data for each status update
            $data = [
                'status_value' => $status_value,
                'user_id' => $user_id,
            ];
        
            // Check if the status already exists for this customer
            $this->db->where('customer_id', $customer_id);
            $this->db->where('status_type', $status_type);
        
            $query = $this->db->get('tblclient_survey_statuses');
        
            if ($query->num_rows() > 0) {
                // Update the existing record without changing the timestamp
                $this->db->where('customer_id', $customer_id);
                $this->db->where('status_type', $status_type);
                $this->db->update('tblclient_survey_statuses', $data);
            } else {
                // Insert a new record with the current timestamp and user ID
                $data['customer_id'] = $customer_id;
                $data['status_type'] = $status_type;
                $data['date_updated'] = date('Y-m-d H:i:s'); // Add timestamp for new record
                $this->db->insert('tblclient_survey_statuses', $data);
            }
        } 
        return true; 
    }

    public function update_survey_status_ajax($client_id, $status_type, $statuses)
    {
        $this->db->trans_start(); // Start transaction
    
        // Prepare data for update/insert
        $data = [
            'status_value' => $statuses['status_value'],
            'date_updated' => $statuses['date_updated'],
            'user_id' => $statuses['user_id']
        ];
    
        // Check if a record exists for this client_id and status_type
    ...
    public function update_workflow_status_ajax($client_id, $status_type, $statuses)
    {
        $this->db->trans_start(); // Start transaction
        ...
        $this->db->where('customer_id', $client_id);
        $this->db->where('status_type', $status_type);
        $query = $this->db->get('tblclient_worflow_statuses');
    
        if ($query->num_rows() > 0) {
            // Record exists, update it
            $this->db->where('customer_id', $client_id);
            $this->db->where('status_type', $status_type);
            $this->db->update('tblclient_worflow_statuses', $data);
        } else {
            // Record does not exist, insert it
            $data['customer_id'] = $client_id;
            $data['status_type'] = $status_type;
            $this->db->insert('tblclient_worflow_statuses', $data);
        }
    ...
    public function update_dwt_status_ajax($client_id, $status_type, $statuses)
    {
        $this->db->trans_start(); // Start transaction
        ...
        $this->db->where('customer_id', $client_id);
        $this->db->where('status_type', $status_type);
        $query = $this->db->get('tblclient_dwp_statuses');
    
        if ($query->num_rows() > 0) {
            // Record exists, update it
            $this->db->where('customer_id', $client_id);
            $this->db->where('status_type', $status_type);
            $this->db->update('tblclient_dwp_statuses', $data);
        } else {
            // Record does not exist, insert it
            $data['customer_id'] = $client_id;
            $data['status_type'] = $status_type;
            $this->db->insert('tblclient_dwp_statuses', $data);
        }
