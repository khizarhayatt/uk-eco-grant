...
class Misc_model extends App_Model
{
    ...
    public function get_comments($rel_id, $rel_type)
    {
        $this->db->join(db_prefix() . 'staff', db_prefix() . 'staff.staffid=' . db_prefix() . 'notes.addedfrom');
        $this->db->where('rel_id', $rel_id);
        $this->db->where('rel_type', $rel_type);
        ...
        $comments = $this->db->get(db_prefix() . 'notes')->result_array();

        return hooks()->apply_filters('get_comments', $comments, ['rel_id' => $rel_id, 'rel_type' => $rel_type]);
    }
    ...
    public function add_comment($data, $rel_type, $rel_id)
    {
        $data['dateadded']   = date('Y-m-d H:i:s');
        $data['addedfrom']   = get_staff_user_id();
        $data['rel_type']    = $rel_type;
    ...
    {
        // Apply filters to data before updating
        $data = hooks()->apply_filters('update_note_data', $data, $id);

        // Execute action before updating the note
        ...
        $this->db->where('id', $id);
        $this->db->where('is_comment', 1);
        $this->db->update(db_prefix() . 'notes', $update_data);

        if ($this->db->affected_rows() > 0) {
            // Execute action after updating the note
            hooks()->do_action('note_updated', $id, $update_data);
    ...
    public function delete_comment($note_id)
    {
        hooks()->do_action('before_delete_note', $note_id);
